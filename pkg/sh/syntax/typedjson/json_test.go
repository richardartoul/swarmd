...
func (osWriterFS) WriteFile(name string, data []byte, perm fs.FileMode) error {
	return os.WriteFile(name, data, perm)
}

var testWriterFS writerFS = osWriterFS{}
...
func TestRoundtrip(t *testing.T) {
	...
	for _, shellPath := range shellPaths {
		...
		t.Run(name, func(t *testing.T) {
			...
			shellInput, err := fs.ReadFile(os.DirFS("."), shellPath)
			qt.Assert(t, qt.IsNil(err))
			jsonInput, err := fs.ReadFile(os.DirFS("."), jsonPath)
			...
			got := sb.String()
			if *update {
				err := testWriterFS.WriteFile(jsonPath, []byte(got), 0o666)
				qt.Assert(t, qt.IsNil(err))
			} else {
