...
type testSetupFS interface {
	MkdirTemp(dir, pattern string) (string, error)
	WriteFile(name string, data []byte, perm fs.FileMode) error
}

type osTestSetupFS struct{}

func (osTestSetupFS) MkdirTemp(dir, pattern string) (string, error) {
	return os.MkdirTemp(dir, pattern)
}

func (osTestSetupFS) WriteFile(name string, data []byte, perm fs.FileMode) error {
	return os.WriteFile(name, data, perm)
}

var setupFS testSetupFS = osTestSetupFS{}
...
func TestMainSetup() {
	...
	pathDir, err := setupFS.MkdirTemp("", "interp-bin-")
	if err != nil {
		panic(err)
	}
	...
	for _, s := range []string{
		...
		if err := setupFS.WriteFile(pathFile, []byte("#!/bin/sh\necho NO_SUCH_COMMAND; exit 1"), 0o777); err != nil {
			panic(err)
		}
	}
