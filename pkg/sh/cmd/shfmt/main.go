...
import (
	...
	"github.com/richardartoul/swarmd/pkg/sh/fileutil"
	"github.com/richardartoul/swarmd/pkg/sh/syntax"
	"github.com/richardartoul/swarmd/pkg/sh/syntax/typedjson"
)
...
func (osHostFS) Open(name string) (fs.File, error) {
	return os.Open(name)
}

func (osHostFS) Stat(name string) (fs.FileInfo, error) {
	return os.Stat(name)
}

func (osHostFS) Lstat(name string) (fs.FileInfo, error) {
...
`)
	...
	for _, path := range flag.Args() {
		if info, err := hostFS.Stat(path); err == nil && !info.IsDir() && !applyIgnore.val && find.val == "false" {
