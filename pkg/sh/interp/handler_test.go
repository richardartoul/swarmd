...
type blocklistNondevFS struct{ interp.OSFileSystem }

func (blocklistNondevFS) Open(path string) (fs.File, error) {
...
func (blocklistNondevFS) OpenFile(path string, flags int, mode os.FileMode) (io.ReadWriteCloser, error) {
	if !interp.IsDevNullPath(path) {
		return nil, fmt.Errorf("non-dev: %s", path)
	}
	return interp.OSFileSystem{}.OpenFile(path, flags, mode)
}
...
type blocklistGlobFS struct{ interp.OSFileSystem }

func (blocklistGlobFS) ReadDir(path string) ([]fs.DirEntry, error) {
...
var modCases = []struct {
	...
	},
	{
		...
		opts: []interp.RunnerOption{
			interp.FileSystem(blocklistNondevFS{}),
		},
		src:  "echo foo >/dev/null; echo bar >/tmp/x",
		want: "Runner.Run error: non-dev: /tmp/x",
	},
	{
		name: "OpenForbidNonDevCoreutils",
		opts: []interp.RunnerOption{
			interp.ExecHandlers(coreutils.ExecHandler),
			interp.FileSystem(blocklistNondevFS{}),
		},
		src:  "cat /dev/null /tmp/x",
		want: "Runner.Run error: coreutils: non-dev: /tmp/x",
	},
	{
		name: "OpenMockFile",
		opts: []interp.RunnerOption{
	...
	},
	{
		name: "GlobForbid",
		opts: []interp.RunnerOption{
			interp.FileSystem(blocklistGlobFS{}),
		},
		src:  "echo *",
