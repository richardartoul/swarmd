...
var testFS = interp.OSFileSystem{}

func mustFile(path string, flags int, perm os.FileMode) (*os.File, error) {
	f, err := testFS.OpenFile(path, flags, perm)
	if err != nil {
		return nil, err
	}
	file, ok := f.(*os.File)
	if !ok {
		_ = f.Close()
		return nil, fmt.Errorf("expected *os.File, got %T", f)
	}
	return file, nil
}
...
func namedSubtest(index int, src string) string {
	src = strings.TrimSpace(src)
	if src == "" {
		return fmt.Sprintf("%03d_empty", index)
	}
	...
	return fmt.Sprintf("%03d_%s", index, src)
}
...
var runTests = []runTest{
	...
	{"foo=bar; foo=x true; echo $foo", "bar\n"},
	{"foo=bar; $ENV_PROG | grep -E '^foo='", "exit status 1"},
	{"foo=bar $ENV_PROG | grep -E '^foo='", "foo=bar\n"},
	{"foo=a foo=b $ENV_PROG | grep -E '^foo='", "foo=b\n"},
	{"$ENV_PROG | grep -i -E '^interp_global='", "INTERP_GLOBAL=value\n"},
	{"INTERP_GLOBAL=new; $ENV_PROG | grep -i -E '^interp_global='", "INTERP_GLOBAL=new\n"},
	{"INTERP_GLOBAL=; $ENV_PROG | grep -i -E '^interp_global='", "INTERP_GLOBAL=\n"},
	{"unset INTERP_GLOBAL; $ENV_PROG | grep -i -E '^interp_global='", "exit status 1"},
	...
	{
		"a=sub true & { a=main $ENV_PROG | grep -E '^a='; }",
		"a=main\n",
	},
	{
	...
	{
		"mkdir a; echo foo >a |& grep -F -q 'is a directory'",
	...
	{
		"set -a; foo=bar; $ENV_PROG | grep -E '^foo='",
		"foo=bar\n",
	},
	{
		"set -a; foo=(b a r); $ENV_PROG | grep -E '^foo='",
		"exit status 1",
	},
	{
		"foo=bar; set -a; $ENV_PROG | grep -E '^foo='",
		"exit status 1",
	},
	{
	...
	{"set -e; set -o | grep -E 'errexit|noexec' | grep -E 'on$' | wc -l | tr -d ' '", "1\n"},
	{
		"set -a; set +o",
		`set -o allexport
set +o errexit
...
set +o pipefail
 #IGNORE`,
	...
	*) echo "invalid option $REPLY"; break;;
	...
	{"set -e; shopt -o | grep -E '^(errexit|noexec)' | grep -E 'on$' | wc -l | tr -d ' '", "1\n"},
	...
	{"shopt -u globstar; shopt globstar | grep -E 'off$' | wc -l | tr -d ' '", "1\n"},
	{"shopt -s globstar; shopt globstar | grep -E 'off$' | wc -l | tr -d ' '", "0\n"},
	{"shopt extglob | grep -F 'off' | wc -l | tr -d ' '", "1\n"},
	{
		"shopt inherit_errexit",
		"inherit_errexit\ton\t(\"off\" not supported)\n #JUSTERR",
	},
	{
		"shopt -o -s pipefail; shopt -o pipefail | grep -q -E 'on$'",
		"",
	},
	{
		"shopt -o -u pipefail; shopt -o pipefail | grep -q -E 'on$'",
		"exit status 1",
	},
	{
		"shopt pipefail",
	...
	{"echo() { :; }; type echo | grep -F 'is a function'", "echo is a function\n"},
	...
	{"type -P $PATH_PROG | grep -q -E '^(/|[A-Z]:)'", ""},
	{"PATH=/; type -P $PATH_PROG", "exit status 1"},
	{"echo() { :; }; type -f echo", "echo is a shell builtin\n"},
	...
	// export
	{"declare foo=bar; $ENV_PROG | grep -E '^foo='", "exit status 1"},
	{"declare -x foo=bar; $ENV_PROG | grep -E '^foo='", "foo=bar\n"},
	{"export foo=bar; $ENV_PROG | grep -E '^foo='", "foo=bar\n"},
	{"foo=bar; export foo; $ENV_PROG | grep -E '^foo='", "foo=bar\n"},
	{"export foo=bar; foo=baz; $ENV_PROG | grep -E '^foo='", "foo=baz\n"},
	{"export foo=bar; readonly foo=baz; $ENV_PROG | grep -E '^foo='", "foo=baz\n"},
	{"export foo=(1 2); $ENV_PROG | grep -E '^foo='", "exit status 1"},
	{"declare -A foo=([a]=b); export foo; $ENV_PROG | grep -E '^foo='", "exit status 1"},
	{"export foo=(b c); foo=x; $ENV_PROG | grep -E '^foo='", "exit status 1"},
	{"foo() { bar=foo; export bar; }; foo; $ENV_PROG | grep -E '^bar='", "bar=foo\n"},
	{"foo() { export bar; }; bar=foo; foo; $ENV_PROG | grep -E '^bar='", "bar=foo\n"},
	{"foo() { export bar; }; foo; bar=foo; $ENV_PROG | grep -E '^bar='", "bar=foo\n"},
	{"foo() { export bar=foo; }; foo; readonly bar; $ENV_PROG | grep -E '^bar='", "bar=foo\n"},
	...
	{
		`export x=before; f() { local x; export x=after; $ENV_PROG | grep -E '^x='; }; f; echo $x`,
		"x=after\nbefore\n",
	},
	{
	...
	{
		"foo() { readonly bar=foo; }; foo; export bar; $ENV_PROG | grep -E '^bar='",
		"bar=foo\n",
	},

	// multiple var modes at once
	{
		"declare -r -x foo=bar; $ENV_PROG | grep -E '^foo='",
		"foo=bar\n",
	},
	{
	...
	{
		"read -pPrompt choice <<< 'y'; echo $choice",
		"Prompty\n #IGNORE bash requires a terminal",
	},
	{
...
var runTestsWindows = []runTest{
	...
	},
	{
		...
		"$ENV_PROG | grep -i -E '^mixedCase_interp'",
	...
	{
		"MIXEDCASE_interp_global=replaced; $ENV_PROG | grep -i -E '^mixedcase_interp'",
...
func TestRunnerRun(t *testing.T) {
	...
	for i, c := range runTests {
		c := c
		t.Run(namedSubtest(i, c.in), func(t *testing.T) {
...
var testBuiltinsMap = map[string]func(interp.HandlerContext, []string) error{
	...
	"sed": func(hc interp.HandlerContext, args []string) error {
		...
		case 2:
			var err error
			f, err = testFS.Open(absPath(hc.Dir, args[1]))
			if err != nil {
				return err
			}
		default:
	...
	"mkdir": func(hc interp.HandlerContext, args []string) error {
		for _, arg := range args {
			...
			path := absPath(hc.Dir, arg)
			if err := testFS.MkdirAll(path, 0o777); err != nil {
				return err
			}
		}
		return nil
	...
	"rm": func(hc interp.HandlerContext, args []string) error {
		for _, arg := range args {
			...
			path := absPath(hc.Dir, arg)
			if err := testFS.RemoveAll(path); err != nil {
				return err
			}
		}
		return nil
	...
	"ln": func(hc interp.HandlerContext, args []string) error {
		...
		if symbolic {
			return testFS.Symlink(oldname, newname)
		}
		return testFS.Link(oldname, newname)
	},
	"touch": func(hc interp.HandlerContext, args []string) error {
		...
		for _, arg := range filenames {
			...
			f, err := testFS.OpenFile(path, os.O_CREATE, 0o666)
			if err != nil {
				return err
			}
			f.Close()
			// change the modification and access time
			if err := testFS.Chtimes(path, newTime, newTime); err != nil {
				return err
			}
		}
		return nil
...
func TestRunnerOpts(t *testing.T) {
	...
	cases := []struct {
		...
		{
			nil,
			"$ENV_PROG | grep -i -E '^interp_global='",
			"INTERP_GLOBAL=value\n",
		},
		{
			opts(withPath()),
...
func TestRunnerContext(t *testing.T) {
	...
	for i, in := range cases {
		in := in
		t.Run(namedSubtest(i, in), func(t *testing.T) {
			file := parse(t, p, in)
...
func TestRunnerDir(t *testing.T) {
	...
	t.Run("SymlinkOrShortPath", func(t *testing.T) {
		...
		if err := testFS.Mkdir(realDir, 0o777); err != nil {
			t.Fatal(err)
		}
		if err := testFS.WriteFile(realFile, []byte(""), 0o666); err != nil {
			t.Fatal(err)
		}
		...
		if runtime.GOOS == "windows" {
			...
			if err := testFS.Symlink(realDir, altDir); err != nil {
				t.Fatal(err)
			}
		}
...
func TestRunnerResetFields(t *testing.T) {
	...
	logFile, err := mustFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_TRUNC, 0o666)
	if err != nil {
		t.Fatal(err)
	}
	defer logFile.Close()
...
func TestMalformedPathOnWindows(t *testing.T) {
	...
	if err := testFS.WriteFile(path, script, 0o777); err != nil {
		t.Fatal(err)
	}
