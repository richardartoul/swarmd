...
import (
	...
	"github.com/richardartoul/swarmd/pkg/sh/pattern"
...
func (r *Runner) redir(ctx context.Context, rd *syntax.Redirect) (io.Closer, error) {
	...
	if rd.N != nil {
		...
		case "2":
			orig = &r.stderr
		default:
			err := fmt.Errorf("unsupported redirect fd: %v", rd.N.Value)
			r.errf("%v\n", err)
			return nil, err
		}
	}
	...
	case syntax.DplOut:
		...
		default:
			err := fmt.Errorf("unsupported redirect target fd: %q", arg)
			r.errf("%v\n", err)
			return nil, err
		}
		return nil, nil
	...
	case syntax.DplIn:
		...
		default:
			err := fmt.Errorf("unsupported redirect source fd: %q", arg)
			r.errf("%v\n", err)
			return nil, err
		}
		return nil, nil
	default:
		err := fmt.Errorf("unsupported redirect operator: %v", rd.Op)
		r.errf("%v\n", err)
		return nil, err
	}
	mode := os.O_RDONLY
...
func (r *Runner) call(ctx context.Context, pos syntax.Pos, args []string) {
	r.callWithOptions(ctx, pos, args, RunSimpleCommandOptions{})
}

func (r *Runner) callWithOptions(ctx context.Context, pos syntax.Pos, args []string, opts RunSimpleCommandOptions) {
	if r.stop(ctx) {
		return
	}
...
func (r *Runner) open(ctx context.Context, path string, flags int, mode os.FileMode, print bool) (io.ReadWriteCloser, error) {
	_ = ctx
	resolvedPath, err := ResolvePath(r.fileSystem, r.Dir, path)
	if err != nil {
		if print {
			r.errf("%v\n", err)
		}
	...
	// TODO: support wrapped PathError returned from the filesystem adapter.
	switch err.(type) {
	case nil:
		return f, nil
	case *os.PathError:
	...
	default: // filesystem adapter's custom fatal error
		r.exit.fatal(err)
	}
	return nil, err
}
...
func (r *Runner) readDir(path string) ([]fs.DirEntry, error) {
	var err error
	path, err = ResolvePath(r.fileSystem, r.Dir, path)
	if err != nil {
		return nil, err
	}
...
func (r *Runner) stat(ctx context.Context, name string) (fs.FileInfo, error) {
	_ = ctx
	name, err := ResolvePath(r.fileSystem, r.Dir, name)
	if err != nil {
		return nil, err
	}
