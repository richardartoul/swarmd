...
type Runner struct {
	...
	// fileSystem is responsible for filesystem operations used by the interpreter.
	// It must not be nil.
	fileSystem RunnerFileSystem

	// networkDialer is responsible for outbound network connections used by
	// interpreter-provided commands. It must not be nil.
	networkDialer NetworkDialer

	// httpClientFactory constructs HTTP clients for interpreter-provided
	// commands like curl.
	...
	shellVariant syntax.LangVariant
...
func New(opts ...RunnerOption) (*Runner, error) {
	r := &Runner{
		usedNew:       true,
		fileSystem:    DefaultRunnerFileSystem(),
		networkDialer: DefaultNetworkDialer(),
		shellVariant:  syntax.LangBash,
	}
	...
	if r.httpClientFactory == nil {
		httpClientFactory, err := NewHTTPClientFactory(r.networkDialer, nil)
		if err != nil {
			return nil, err
		}
		r.httpClientFactory = httpClientFactory
	}
	if tempDir := runnerTempDir(r.fileSystem, r.Env); tempDir != "" {
		if mutableFS, ok := r.fileSystem.(MutableFileSystem); ok {
			if err := mutableFS.MkdirAll(tempDir, 0o755); err != nil {
...
func runnerTempDir(fsys RunnerFileSystem, env expand.Environ) string {
	if dir := env.Get("TMPDIR").String(); dir != "" {
		baseDir, err := fsGetwd(fsys)
		if err == nil {
			resolved, err := ResolvePath(fsys, baseDir, dir)
			if err == nil {
				return resolved
			}
		}
	}
	return fsTempDir(fsys)
}
...
func Dir(path string) RunnerOption {
	return func(r *Runner) error {
		...
		baseDir := r.Dir
		if baseDir == "" {
			baseDir, err = fsGetwd(r.fileSystem)
			if err != nil {
				return fmt.Errorf("could not get current dir: %w", err)
			}
		}
		path, err := ResolvePath(r.fileSystem, baseDir, path)
		if err != nil {
			return fmt.Errorf("could not resolve dir: %w", err)
		}
...
// ShellVariant configures which builtin surface the interpreter should expose.
// The parser should generally use the same language variant.
func ShellVariant(lang syntax.LangVariant) RunnerOption {
	return func(r *Runner) error {
		r.shellVariant = lang
		return nil
	}
	...
	return func(r *Runner) error {
		...
		for i < len(args) {
			...
			if arg == "-" {
				...
				if params := args[i+1:]; len(params) > 0 {
					r.Params = params
				}
				return nil
			}
			...
			for len(rest) > 0 {
				...
				rest = rest[1:]
				if flag != 'o' {
					opt := r.posixOptByFlag(flag)
					if opt == nil {
						return fmt.Errorf("invalid option: %q", string(arg[0])+string(flag))
					}
					*opt = enable
					continue
				}
...
// TODO: consider porting the middleware API in [ExecHandlers] to filesystem operations.
...
// FileSystem sets the filesystem used by the interpreter.
func FileSystem(fsys RunnerFileSystem) RunnerOption {
	return func(r *Runner) error {
		if fsys == nil {
...
// Network sets the network dialer used by the interpreter.
func Network(dialer NetworkDialer) RunnerOption {
	return func(r *Runner) error {
		if dialer == nil {
			return fmt.Errorf("network dialer must not be nil")
		}
		r.networkDialer = dialer
		return nil
	}
}

// HTTPClient sets the HTTP client factory used by interpreter-owned commands.
func HTTPClient(factory HTTPClientFactory) RunnerOption {
	return func(r *Runner) error {
		if factory == nil {
			return fmt.Errorf("HTTP client factory must not be nil")
		}
		r.httpClientFactory = factory
		return nil
	}
}
...
func (r *Runner) Reset() {
	...
	*r = Runner{
		...
		networkDialer:     r.networkDialer,
		httpClientFactory: r.httpClientFactory,
